...
import { mapToModel } from "./picService.js";
...
export const onLogin = (email, password, users = []) => {
  ...
  LOGOUT_LINK_CONTAINER.className = "d-block";

  LOGIN_LINK.className = "d-none";
  SIGNUP_PAGE_LINK.className = "d-none";

  EDIT_USER_LINK.innerHTML = user.email;
...
const editUserListeners = () => {
  ...
  FIRST_EDIT_USER_FIELD.addEventListener("input", (e) =>
    onChangeInputField(
      schema,
      {
...
export const mapToUserModel = (users, id) => {
  const user = users.find((user) => user._id === id);
  if (!user) throw new Error("Opss... there is no user with this id: " + id);
  const {
    firstedit,
    lastedit,
    stateedit,
  ...
  COUNTRY_EDIT_USER_FIELD.value = countryedit;
  CITY_EDIT_USER_FIELD.value = cityedit;
  STREET_EDIT_USER_FIELD.value = streetedit;
  HOUSE_EDIT_USER_FIELD.value = houseedit;
  ZIP_EDIT_USER_FIELD.value = zipedit;
  PHONE_EDIT_USER_FIELD.value = phoneedit;
  BIZ_EDIT_USER_FIELD.value = isBizedit;
};
...
export const handleEditUser = () => {
  ...
  console.log(userId);
  console.log(user);
  mapToUserModel(users, userId);
  editUserListeners();
  // CANCEL_BTN_EDIT_USER.addEventListener("click", console.log("submit"));
  // SUBMIT_BTN_EDIT_USER.addEventListener("click", console.log("cancel"));
};
